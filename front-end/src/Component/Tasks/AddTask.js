...
function AddTask({ onClose }) {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/work/tasks/get-members`, {
          params: { CurrentUser },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/work/tasks/Add-Task", formData, {
        params: { CurrentUser },
      });
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[#e7edf9] dark:bg-[#010B13] p-10 rounded-lg shadow-lg w-full max-w-xl">
        <form onSubmit={handleSubmit}>
          <div className="mb-6">
            <label className="block text-sm font-semibold text-gray-700 dark:text-[#F8F8FF] mb-1">
              Task Title:
            </label>
            <input
              type="text"
